...
Full text of decision-log entries relocated verbatim from
[`decisions.md`](./decisions.md) under the bounded-log protocol
([`../README.md`](../README.md)): a wave/audit entry is collapsed in the live log once
its `docs/audit-*.md` report exists and all its items are closed or re-tracked there.
Nothing is ever deleted — archiving relocates history, mirroring the repo's
merge-preserves-history rule. Newest first.
