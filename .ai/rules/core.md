...
* **Language & Version Control Protocol:** All generated code, variables, and inline comments MUST be written strictly in professional English. Git commit messages MUST follow the **Conventional Commits** specification (e.g., `feat:`, `fix:`, `refactor:`). Git commit messages MUST NOT carry AI-authorship trailers or credits — no `Co-Authored-By:` line naming an AI agent (e.g. Claude), and no "Generated with…" or "Co-authored with an AI" note. Every commit is authored solely by Nicolas. Conversational explanations in the chat MUST match the user's prompt language.
