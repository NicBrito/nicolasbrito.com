                        ...
                        <ul className="touch-desktop-nav items-center gap-[clamp(1.5rem,4vw,3rem)] flex-wrap justify-center">
                            {NAV_ITEMS.map((item, index) => (
                                <li key={item.key} className="relative">
                                    <Link
                                        ref={(el) => {
                                            if (el) navItemRefs.current.set(item.key, el as HTMLAnchorElement);
                                            else navItemRefs.current.delete(item.key);
                                        }}
                                        href={item.href}
                                        data-nav-item
                                        tabIndex={0}
                                        ...
                                        <motion.div variants={ANIMATIONS.contentItem}>
                                            <Link
                                                ref={(el) => {
                                                    ...
                                                    else exploreItemRefs.current.delete(`${activeMenu}-explore`);
                                                }}
                                        ...
                                        <div className="flex flex-col gap-3.5 relative">
                                            {currentMenu.items.map((itemKey, index) => (
                                                ...
                                                >
                                                    <Link
                                                        ref={(el) => {
                                                            if (el) dropdownItemRefs.current.set(`${activeMenu}-${itemKey}`, el as HTMLAnchorElement);
                                                            else dropdownItemRefs.current.delete(`${activeMenu}-${itemKey}`);
                                                        }}
                                                        href={`${currentMenu.href}`}
                                                        data-dropdown-item
                                                        tabIndex={0}
